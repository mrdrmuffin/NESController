//-----------------------------------------------------------------------------
// F3xx_USB0_Standard_Requests.U8
//-----------------------------------------------------------------------------
// Copyright 2010 Silicon Laboratories, Inc.
// http://www.silabs.U8om
//
// Program Description:
//
// This source file contains the subroutines used to handle incoming
// setup packets. These are called by Handle_Setup in USB_ISR.U8 and used for
...
//    -02 APR 2010
// Release 1.1
//    - Minor changes to F3xx_USB0_Descriptor.U8
//    - 16 NOV 2006
// Release 1.0
//    -Initial Revision (PD)
//    -07 DEC 2005
...
// These are created in USB0_DESCRIPTOR.U8

extern SEGMENT_VARIABLE(DEVICEDESC, const device_descriptor, SEG_CODE);
extern SEGMENT_VARIABLE(HIDCONFIGDESC, const hid_configuration_descriptor, SEG_CODE);
extern SEGMENT_VARIABLE(HIDREPORTDESC, const hid_report_descriptor, SEG_CODE);
extern SEGMENT_VARIABLE_SEGMENT_POINTER(STRINGDESCTABLE[], U8, const SEG_CODE, const SEG_CODE);

// These are created in F3xx_USB0_InterruptServiceRoutine.U8
   ...
   {                                   // was requested, and set data ptr and
     ...
     case DSC_STRING:
       //  NOTE: if strings are added to this project, the hard-coded
         ...
         if (SETUP.wValue.U8[LSB] > 2)   // If asking for string that's N/A
         {
            Force_Stall();
         }
         else
      ...
      case DSC_ENDPOINT:
         ...
         // In the ...Descriptor.U8 and ...Descriptor.h files,
